            ...
            try {
                if (r1.next()) {
                    ...
                    if (disponivel.compareTo(BigDecimal.ZERO) < 0) {
                        throw new Exception("<font size='12'><b> Estoque insuficiente para atender a solicitação. <br> Disponível: "
                                + estoque + " para o local " + iteVO.asBigDecimal("CODLOCALORIG") + "<br><br> Favor verificar se há quantidade no local ou empresa solicitados. </b></font>");
                    }
                }
            } finally {
